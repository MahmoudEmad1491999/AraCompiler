    ...
    public class Tokenizer
    {
        ...
        private Token getNextToken()
        {
            if (translationUnit.hasnext())
            {
                ...
                else if (translationUnit.nextIsADigit())
                {
                    return handleADigit();
                }
                else if (translationUnit.nextIsEDigit())
                {
                    return handleEDigit();
                }
        ...
        private Token handleADigit()
        {
            StringBuilder digit_string_builder = new StringBuilder();
            while (translationUnit.nextIsADigit()) digit_string_builder.Append(translationUnit.getNextChar());
            string digit_string = digit_string_builder.ToString();
            return new Token(TokenType.INT_LITERAL, translationUnit.countedLines, digit_string);
        }
        private Token handleEDigit()
        {
            StringBuilder digit_string_builder = new StringBuilder();
            while (translationUnit.nextIsEDigit()) digit_string_builder.Append(translationUnit.getNextChar());
            string digit_string = digit_string_builder.ToString();
            return new Token(TokenType.INT_LITERAL, translationUnit.countedLines, digit_string);
        }
    }
